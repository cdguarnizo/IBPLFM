function [model, options, params] = ibpmultigpOptimise(model, display, iters)
...
if strcmp(func2str(optim), 'optimiMinimize')
  ...
  params = optim('ibpmultigpObjectiveGradient', params, options, model);
elseif strcmp(func2str(optim), 'rprop')
    params = optim(params, 'ibpmultigpObjectiveGradient', iters, model);
elseif strcmp(func2str(optim), 'scg2')
    [params, options] = optim('ibpmultigpObjectiveGradient', params,  options, ...
                 'ibpmultigpGradient', model);
else
  % NETLAB style optimization.
  [params, options] = optim('ibpmultigpObjective', params,  options, ...
                 'ibpmultigpGradient', model);
end
