...
if isfield(model,'Opteta') && model.Opteta,
    getadqParams = ibpmultigpLowerBoundGradeta(model);
    fhandle = str2func([model.etadqTransform 'Transform']);
    getadqParams = getadqParams.*fhandle(model.etadqvec, 'gradfact');
else
    getadqParams = [];
end
...
if isfield(model, 'UseMeanConstants') && model.UseMeanConstants,
    %gradients wrt mudq
    gmudqParams = zeros(1,model.nout);
    temp = model.m2'*model.Ainv;
    for d = 1:model.nout,
        Psit = model.etadq(d,model.indXu)'.*cell2mat(model.Psit(d,:)');
        gmudqParams(d) = -model.beta(d)*(temp*Psit...
            + model.sizeX(d)*model.mu(d));
    end
else
    gmudqParams = [];
end

gParam = [gKernParam gGamma gBeta gadqparams(:)' gbdqparams(:)' gX getadqParams gmudqParams];
